...
"""
def get_aim_UID_with_label_rdd(Spark_Session, Label_Data_Config_dict, tmp_aim_time_start_or_list, tmp_aim_time_end, tmp_store_dir, 
    ...
    if type(tmp_aim_time_start_or_list) == str:
        tmp_aim_time_start = tmp_aim_time_start_or_list
    elif type(tmp_aim_time_start_or_list) == list:
        tmp_aim_time_start = tmp_aim_time_start_or_list[0][0].strftime("%Y-%m-%d")
    ...
    if not os.path.exists(tmp_output_data_label_file):
        ...
        if type(tmp_aim_time_start_or_list) == str:
            tmp_time_limit = """(""" + Aim_Time_Column + """ >= '""" + tmp_aim_time_start_or_list + """'"""
            tmp_time_limit = tmp_time_limit + """ AND """ + Aim_Time_Column + """ <  '""" + tmp_aim_time_end + """')"""
        elif type(tmp_aim_time_start_or_list) == list:
            tmp_time_limit = """("""
            for tmp_time_range_i in range(len(tmp_aim_time_start_or_list)):
                tmp_time_range = tmp_aim_time_start_or_list[tmp_time_range_i]
                
                tmp_sub_aim_time_start = tmp_time_range[0].strftime("%Y-%m-%d")
                tmp_sub_aim_time_end = tmp_time_range[1].strftime("%Y-%m-%d")
                
                if tmp_time_range_i > 0:
                    tmp_time_limit = tmp_time_limit + """ OR """
                
                tmp_time_limit = tmp_time_limit + """(""" + Aim_Time_Column + """ >= '""" + tmp_sub_aim_time_start + """'"""
                tmp_time_limit = tmp_time_limit + """ AND """ + Aim_Time_Column + """ <  '""" + tmp_sub_aim_time_end + """')"""
                
            tmp_time_limit = tmp_time_limit + """)"""
            
        tmp_sql_command = tmp_sql_command + """\n    AND """ + tmp_time_limit
            
        if 'Label_Map_Keys' in Label_Data_Config_dict:
            tmp_aim_label_list = Label_Data_Config_dict['Label_Map_Keys']
            tmp_aim_label_list = [str(x) for x in tmp_aim_label_list]
            
            tmp_sql_command = tmp_sql_command + """\n    AND """ + Aim_Label_Column + """ IN (""" + ','.join(tmp_aim_label_list) + """)"""
        else:
        ...
        # 对标签进行转化
        if 'Label_Map_Keys' in Label_Data_Config_dict:
            tmp_aim_entity_rdd = tmp_aim_entity_rdd.replace(Label_Data_Config_dict['Label_Map_Keys'], 
