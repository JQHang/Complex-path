...
HAN_Config_dict = {}
HAN_Config_dict['train_sample_size'] = 2048
HAN_Config_dict['eval_sample_size'] = 100000
HAN_Config_dict['train_pos_sample_percent'] = 0.1
HAN_Config_dict['train_epoch'] = 1000
HAN_Config_dict['early_stop'] = 20
HAN_Config_dict['sample_num_for_eval'] = 100
# HAN_Config_dict['learning_rate'] = 0.0001
HAN_Config_dict['learning_rate'] = 0.0001
# HAN_Config_dict['weight_decay'] = 1e-4
HAN_Config_dict['weight_decay'] = 0

HAN_Config_dict['node_feature_hid_len'] = 128
HAN_Config_dict['metapath_level_nhid'] = 128
HAN_Config_dict['num_Res_DNN'] = 1
HAN_Config_dict['each_Res_DNN_num'] = 2
# HAN_Config_dict['dropout'] = 0.2
HAN_Config_dict['dropout'] = 0.1

print(HAN_Config_dict)


# In[9]:


# {'train_sample_size': 20000, 'eval_sample_size': 40000, 'train_pos_sample_percent': 0.2, 'train_epoch': 1000, 'early_stop': 30, 'sample_num_for_eval': 30, 'learning_rate': 5e-05, 'weight_decay': 0, 'node_feature_hid_len': 128, 'metapath_level_nhid': 128, 'num_Res_DNN': 2, 'each_Res_DNN_num': 3, 'dropout': 0}
...
from kg_lib.Get_HAN_Required_Data import Get_HAN_Required_Data_and_Store

# 各时间区间对应的数据
time_range_to_Processed_HAN_Data_dict = {}
...
for tmp_aim_time_monthly_range in all_aim_time_monthly_range_list:
    ...
    Processed_HAN_Data_dict = Get_HAN_Required_Data_and_Store(data_source_description_str, time_range_str,
                                                              aim_node_type = Label_Data_Config_dict['Node_Type'], 
                                                              Meta_path_drop_list = [])
    
    if len(node_type_to_feature_len_dict.keys()) == 0:
        for tmp_node_type in Processed_HAN_Data_dict['Feature']:
            node_type_to_feature_len_dict[tmp_node_type] = Processed_HAN_Data_dict['Feature'][tmp_node_type].shape[1]
        
        all_meta_path_list = list(Processed_HAN_Data_dict['Adj'].keys())
    
    time_range_to_Processed_HAN_Data_dict[time_range_str] = Processed_HAN_Data_dict
...
def sample_random_index_with_portion(Processed_HAN_Data_dict, sample_size = 3000, positive_percent = 0.2):
    tmp_pos_sample_size = math.ceil(sample_size * positive_percent)
    tmp_neg_sample_size = (sample_size - tmp_pos_sample_size)

    # 获取正样本的序号
    tmp_pos_sample_index_np = np.argwhere(Processed_HAN_Data_dict['Label'] == 1).T[0]

    # 随机选取指定数目的正样本的序号
    tmp_sub_pos_sample_index_np = np.random.choice(tmp_pos_sample_index_np, size = tmp_pos_sample_size, replace = False)

    # 获取负样本的序号
    tmp_neg_sample_index_np = np.argwhere(Processed_HAN_Data_dict['Label'] == 0).T[0]
...
def sample_sub_graph_with_label_index(Processed_HAN_Data_dict, tmp_head_node_type, tmp_sampled_label_index):
    sub_graph_data_dict = {}
    ...
    sub_graph_data_dict['Label'] = torch.FloatTensor(Processed_HAN_Data_dict['Label'][tmp_sampled_label_index])
    if args_cuda:
...
from kg_model.HAN import HAN

# 建立模型
model = HAN(node_type_to_feature_len_dict, all_meta_path_list, node_feature_hid_len = HAN_Config_dict['node_feature_hid_len'], 
            metapath_level_nhid = HAN_Config_dict['metapath_level_nhid'], num_Res_DNN = HAN_Config_dict['num_Res_DNN'], 
            each_Res_DNN_num = HAN_Config_dict['each_Res_DNN_num'], dropout = HAN_Config_dict['dropout'])
...
# 优化器
optimizer = optim.Adam(model.parameters(), lr = HAN_Config_dict['learning_rate'], weight_decay = HAN_Config_dict['weight_decay'])
...
def evaluate(model, source_Data_Dict, need_transfer = True):
    ...
    with torch.no_grad():    
        if need_transfer:
            ...
            for sample_start in tqdm(range(0, source_Data_Dict['Label'].shape[0], HAN_Config_dict['eval_sample_size'])):
                sample_end = sample_start + HAN_Config_dict['eval_sample_size']
                if sample_end > source_Data_Dict['Label'].shape[0]:
                    sample_end = source_Data_Dict['Label'].shape[0]
            
                Sampled_Data_Dict = sample_sub_graph_with_label_index(source_Data_Dict, Label_Data_Config_dict['Node_Type'], 
...
def evaluate_multi_time(model, tmp_time_range_list, need_transfer = True, print_figure = False):
    ...
    with torch.no_grad():    
        ...
        for tmp_time_range in tmp_time_range_list:
            ...
            source_Data_Dict = time_range_to_Processed_HAN_Data_dict[time_range_str]
            for sample_start in tqdm(range(0, source_Data_Dict['Label'].shape[0], HAN_Config_dict['eval_sample_size'])):
                sample_end = sample_start + HAN_Config_dict['eval_sample_size']
                if sample_end > source_Data_Dict['Label'].shape[0]:
                    sample_end = source_Data_Dict['Label'].shape[0]
...
# 模型参数的输出文件夹
tmp_model_parameter_output_dir = '../../Model_Parameter/HAN/' + localtime + '_' + temp_train_list_name + '/'
mkdir('../../Model_Parameter')
mkdir('../../Model_Parameter/HAN/')
...
for epoch in range(HAN_Config_dict['train_epoch']):
    # 多少轮查看一次效果
    pbar = tqdm(range(HAN_Config_dict['sample_num_for_eval']))
    for sample_index in pbar:
        ...
        sampled_label_index = sample_random_index_with_portion(time_range_to_Processed_HAN_Data_dict[time_range_str],
                                                               HAN_Config_dict['train_sample_size'],
                                                               HAN_Config_dict['train_pos_sample_percent'])
        Sampled_Data_Dict = sample_sub_graph_with_label_index(time_range_to_Processed_HAN_Data_dict[time_range_str], 
                                                              Label_Data_Config_dict['Node_Type'], 
                                                              sampled_label_index)
    ...
    if val_roc_auc > best_roc_auc:
        ...
        if early_stop_count >= HAN_Config_dict['early_stop']:
            break
