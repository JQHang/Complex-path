...
*Meta-Path: A \MetaPathName \\(P\\) is a path defined on the network schema \\(HG = (V, E)\\) denoted in the form of \\(o_1 \xrightarrow{l_{1}} o_2 \xrightarrow{l_{2}} \cdots \xrightarrow{l_{m-1}} o_{m}\\), where \\(o\\) and \\(l\\) are node types and edge types, respectively.*
